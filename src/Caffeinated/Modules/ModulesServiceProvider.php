...
class ModulesServiceProvider extends ServiceProvider
{
    ...
    protected function registerConsoleCommands()
    {
        ...
        $this->registerMakeExistingMigrationCommand();
    ...
    protected function registerMakeCommand()
    {
        $this->app->bindShared('modules.make', function ($app) {
            $handler = new Handlers\ModuleMakeHandler($app['modules'],
                $app['files']);
     ...
     */
    protected function registerMakeExistingMigrationCommand()
    {
        $this->app->bindShared('modules.makeExistingMigration',
            function ($app) {
                $handler
                    = new Handlers\ModuleMakeExistingMigrationHandler($app['modules'],
                    $app['files']);

                return new Console\ModuleMakeExistingMigrationCommand($handler);
            });
    }
    ...
    protected function registerMakeMigrationCommand()
    {
        $this->app->bindShared('modules.makeMigration', function ($app) {
            $handler = new Handlers\ModuleMakeMigrationHandler($app['modules'],
                $app['files']);
    ...
    protected function registerMakeRequestCommand()
    {
        $this->app->bindShared('modules.makeRequest', function ($app) {
            $handler = new Handlers\ModuleMakeRequestHandler($app['modules'],
                $app['files']);
    ...
    protected function registerMigrateCommand()
    {
        $this->app->bindShared('modules.migrate', function ($app) {
            return new Console\ModuleMigrateCommand($app['migrator'],
                $app['modules']);
        });
    }
    ...
    {
        $this->app['router']->middleware('caffeinated.module',
            'Caffeinated\Modules\Middleware\ModuleMiddleware');
    }
}
